...
- [~] distribution.6 Canary channel — `canary.yml` ships bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (GoReleaser snapshot, sha-stamped `dig --version`), `@vllnt/dig@canary` (npm), and `vllnt-dig` `.devN` (PyPI). The CLI canary runs on `GITHUB_TOKEN` alone; the npm/PyPI canaries publish via OIDC trusted publishing, gated on the `CANARY_ENABLED` repo variable until trusted publishers are configured. Stable stays tag-driven. Validated: GoReleaser snapshot + actionlint. Runbook in [docs/RELEASING.md](docs/RELEASING.md). Remaining: enable OIDC + flip `CANARY_ENABLED` (2026-06-13)
