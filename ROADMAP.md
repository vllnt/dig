...
- [~] distribution.1 SDK registries — npm + PyPI publish CI wired (npm-publish.yml → `@vllnt/dig`, pypi-publish.yml → `dig-client`; both build dig, test the SDK against a real daemon, publish on a GitHub release; gated on NPM_TOKEN / PYPI_TOKEN secrets). Remaining: add the tokens + first publish of each (2026-06-12)
- [~] distribution.6 Canary channel — `canary.yml` ships bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (GoReleaser snapshot, sha-stamped `dig --version`), `@vllnt/dig@canary` (npm), and `dig-client` `.devN` (PyPI). The CLI canary runs on `GITHUB_TOKEN` alone; the npm/PyPI canaries publish via OIDC trusted publishing, gated on the `CANARY_ENABLED` repo variable until trusted publishers are configured. Stable stays tag-driven. Validated: GoReleaser snapshot + actionlint. Runbook in [docs/RELEASING.md](docs/RELEASING.md). Remaining: enable OIDC + flip `CANARY_ENABLED` (2026-06-13)
