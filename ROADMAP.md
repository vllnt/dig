...
- [~] distribution.1 SDK registries — npm in one `npm.yml` (canary on push + release on tag, both OIDC trusted publishing — mirrors @vllnt/ui's publish.yml); `dig-client` via canary.yml + pypi-publish.yml. Both build dig + test the SDK against a real daemon. First `@vllnt/dig` published manually (0.1.0-canary, via clients/typescript/scripts/first-publish.sh, 2026-06-13). Remaining: npm Trusted Publisher (→ npm.yml) + PyPI publisher + first PyPI publish (2026-06-13)
- [~] distribution.6 Canary channel — bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (canary.yml, GoReleaser snapshot, sha-stamped `dig --version`), `@vllnt/dig@canary` (npm.yml), and `dig-client` `.devN` (canary.yml). The CLI canary runs on `GITHUB_TOKEN` alone; the npm/PyPI canaries publish via OIDC trusted publishing, gated on the `CANARY_ENABLED` repo variable. Stable stays tag-driven. Validated: GoReleaser snapshot + actionlint; first npm canary live. Runbook in [docs/RELEASING.md](docs/RELEASING.md). Remaining: OIDC trusted publishers (2026-06-13)
