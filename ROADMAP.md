...
- [~] distribution.6 Canary channel — bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (canary.yml, GoReleaser snapshot, sha-stamped `dig --version`), `@vllnt/dig@canary` (npm.yml), and `dig-client` `.devN` (pypi.yml). CLI canary on `GITHUB_TOKEN`; npm/PyPI canaries via OIDC, gated on `CANARY_ENABLED`. Provenance/attestations on now that the repo is public (sigstore needs public). Stable tag-driven, gated behind maintainer approval (canary-only policy, .claude/rules/release.md). Runbook: [docs/RELEASING.md](docs/RELEASING.md). Remaining: PyPI publisher (2026-06-13)
