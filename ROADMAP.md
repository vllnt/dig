...
- [ ] site-growth.1 Harness registry — `web/lib/harnesses.ts`, one typed source of truth (slug · name · status `stable`|`wip` · install snippets · MCP JSON · what-it-unlocks · doc link) feeding the selector + per-harness pages + sitemap (DRY). v1 stable: claude-code (plugin), cursor (rule), codex (AGENTS.md), generic MCP, AI SDK — all backed by files in-repo. WIP-badged: pi, opencode, native codex plugin (MCP fallback shown). "Mistral vibe" = Mistral as an OpenAI-compatible LLM **endpoint**, documented as a model guide, not a harness tab
- [ ] site-growth.2 Harness selector — `web/components/integrations/harness-picker.tsx` (client): pick an agent → render its exact 10s install (Claude Code → `/plugin marketplace add vllnt/dig` + `/plugin install dig@dig`; any MCP client → the `dig mcp` JSON config); placed in a homepage "Works with your agent" section + on `/install`; Playwright E2E (pick → assert snippet) + visual
- [ ] site-growth.3 Hero reframe (dual-track, lead agent) — homepage leads with persistent-local-memory-for-your-coding-agent + an "Add to your agent" CTA + the 98% hit@5 proof chip (site-launch.6); the file-organizer story moves below as a use-case; nav += Integrations; FAQ += "works with Claude Code / Codex / MCP?"; copy synced into `web/messages/en.json`
- [ ] site-growth.4 /integrations hub — grid of every harness (status badges) linking to its page; server-rendered, nav-linked, sitemap entry
- [ ] site-growth.5 MDX SEO engine — `@next/mdx` + a `gray-matter` glob loader (`web/lib/content.ts`) turning `web/content/{integrations,compare,use-cases,learn}/*.mdx` frontmatter into a typed list; dynamic `[cluster]/[slug]` route with `generateStaticParams` + per-page `generateMetadata` (canonical + OG); `sitemap.ts` auto-extends from the loader (no hand-listing); JSON-LD (SoftwareApplication / FAQPage / HowTo) on templates. en-only for now (`LOCALES=["en"]`)
- [ ] site-growth.6 Integrations cluster (9 pages) — per-harness MDX sourced from docs/integration.md + .claude-plugin/: claude-code, codex, cursor, pi, opencode, gemini, generic MCP, AI SDK, `dig serve`
- [ ] site-growth.7 Comparisons cluster (7 pages) — dig vs mem0 · Letta/MemGPT · LlamaIndex · Obsidian · Zep · vector-DB · RAG-SaaS, sourced from docs/landscape.md (already written)
- [ ] site-growth.8 Concepts/learn cluster (10 pages) — agent memory, MCP, hybrid retrieval (FTS+vector RRF), content-addressed store, drift detection, recall@k/hit@5, local-first AI, reversible changesets, …; sourced from architecture.md + evals.md
- [ ] site-growth.9 Use-cases cluster (8 pages) — agent memory, RAG over local files, dedupe a doc library, organize an Obsidian vault, PARA/GTD/Zettelkasten, research notes, meeting transcripts, second brain
