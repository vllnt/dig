...
- [~] harness-plugins.2 claude-code plugin — `.claude-plugin/plugin.json` ships the dig skill (hp.8, auto-discovered) + the `dig mcp` server (inline mcpServers, so it doesn't auto-load in dig's own repo sessions) + a SessionEnd retention hook (hooks/hooks.json → retain-session.sh; double opt-in, fail-open) that captures finished sessions into memory (agent-memory.1); `/plugin install dig@dig`. Remaining: slash commands (2026-06-13)
...
- [~] harness-plugins.14 codex plugin — `.codex-plugin/plugin.json` packages the dig skill (hp.8, `./skills/`) + the `dig mcp` server (hp.7) as a first-class Codex plugin, with a repo-scoped `.agents/plugins/marketplace.json` (`codex plugin marketplace add vllnt/dig` → `/plugins`). MCP config is `mcp.json`, deliberately **not** `.mcp.json` — a root `.mcp.json` would auto-load the dig server into Claude Code sessions in dig's own repo, the same reason hp.2 keeps mcpServers inline. E2E (`internal/cli/codex_plugin_e2e_test.go`) parses the whole install graph and boots the shipped `dig mcp` over real stdio, driving initialize → tools/list (no mocks). Remaining: the SessionEnd retention hook is deferred — Codex exposes no session-end event (events stop at `Stop`) and `dig retain --transcript` renders Claude JSONL only; it lands with the Codex transcript renderer (agent-memory.1). The AGENTS.md shim (hp.4) stays the baseline; this is the packaged parity surface (2026-06-14)
...
- [~] agent-memory.2 Transcript split — recall now lands on the exact exchange: `recall` returns the most query-relevant window of each matched document (reuses the shared chunker + query-term coverage scoring), not the document head, so recalling a long session surfaces the matching turn instead of its opening lines; deterministic, budget-respecting, falls back to the head on no match. Remaining: index-time turn/round segmentation that also sharpens FTS ranking and feeds composable-pipeline chunking (2026-06-13)
...
- [~] integrations.3 Python SDK — `dig-client` (clients/python): stdlib-only (urllib) typed client over the `dig serve` daemon, same surface as the TS SDK incl. recall/retain memory; CI-tested against a real spawned daemon (no mocks); PyPI-publish CI gated on PYPI_TOKEN (recall/retain added 2026-06-13). Remaining: LangChain/LlamaIndex adapters build on it
- [~] integrations.4 Vercel AI SDK adapter — `@vllnt/dig/ai` exports `digTools(client)` → AI SDK `tool()` defs (zod) for the dig surface incl. `dig_recall` + `dig_retain` (dig as agent memory the model reads/writes as tools), mutations apply-gated; tested via tools' execute() against a real daemon incl. a retain→recall round-trip; example in the SDK README (memory tools added 2026-06-13). Remaining: an automatic retain-after middleware (the tools cover the explicit path)
...
**Exit criteria:** `npm i @vllnt/dig`, `pip install dig-client`, and `brew install dig` all resolve; `dig mcp` is in the public MCP registry; dig appears in the LangChain / LlamaIndex / Vercel AI SDK directories; every entry links the benchmark leaderboard.

- [~] distribution.1 SDK registries — one workflow per package (`npm.yml`, `pypi.yml`), each canary-on-push + release-on-tag via OIDC trusted publishing (no tokens), mirroring @vllnt/ui's publish.yml; both build dig + test the SDK against a real daemon. First `@vllnt/dig@0.1.0-canary` published (manual, clients/typescript/scripts/first-publish.sh, 2026-06-13). Remaining: PyPI trusted/pending publisher + first dig-client publish (npm Trusted Publisher → npm.yml live) (2026-06-13)
- [~] distribution.6 Canary channel — bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (canary.yml, GoReleaser snapshot, sha-stamped `dig --version`), `@vllnt/dig@canary` (npm.yml), and `dig-client` `.devN` (pypi.yml). CLI canary on `GITHUB_TOKEN`; npm/PyPI canaries via OIDC, gated on `CANARY_ENABLED`. Provenance/attestations off while the repo is private (sigstore needs public). Stable tag-driven, gated behind maintainer approval (canary-only policy, .claude/rules/release.md). Runbook: [docs/RELEASING.md](docs/RELEASING.md). Remaining: PyPI publisher; re-enable provenance when public (2026-06-13)
...
**Goal:** dig.vllnt.com leads with the agent-integration story — a dev gives their coding agent persistent, local, reversible memory in ~10 seconds via a harness selector — and a long-tail MDX SEO engine ranks dig for the integration, comparison, concept, and use-case queries devs actually search. Dual-track: agent-memory first, the file-organizer story kept as a use-case. (Builds on the shipped surface — `dig mcp`, the Claude plugin, the cursor/AGENTS.md shims, the SDKs — which `site-launch` never surfaced.)
**Exit criteria:** the homepage leads with the agent-memory value prop and a working harness selector that emits the exact 10s install per shipped harness; an `/integrations` hub + per-harness pages are live; the MDX engine renders all four clusters with sitemap entries auto-generated from content; every agent-integration claim on the site matches the shipped surface; Playwright E2E + 3-viewport visual checks green.

- [x] site-growth.1 Harness registry — `web/lib/harnesses.ts`, one typed source of truth (slug · name · status `stable`|`wip` · install snippets · MCP JSON · what-it-unlocks · doc link) feeding the selector + per-harness pages + sitemap (DRY). v1 stable: claude-code (plugin), cursor (rule), codex (AGENTS.md), generic MCP, AI SDK — all backed by files in-repo. WIP-badged: pi, opencode, native codex plugin (MCP fallback shown). "Mistral vibe" = Mistral as an OpenAI-compatible LLM **endpoint**, documented as a model guide, not a harness tab — shipped #53 (5714f51): harnesses.ts, 8 harnesses (claude-code/codex/cursor/mcp/gemini/ai-sdk stable, pi/opencode wip) (2026-06-14)
- [x] site-growth.2 Harness selector — `web/components/integrations/harness-picker.tsx` (client): pick an agent → render its exact 10s install (Claude Code → `/plugin marketplace add vllnt/dig` + `/plugin install dig@dig`; any MCP client → the `dig mcp` JSON config); placed in a homepage "Works with your agent" section + on `/install`; Playwright E2E (pick → assert snippet) + visual — shipped #53: harness-picker.tsx + tests/e2e/integrations.spec.ts (2026-06-14)
- [x] site-growth.3 Hero reframe (dual-track, lead agent) — homepage leads with persistent-local-memory-for-your-coding-agent + an "Add to your agent" CTA + the 98% hit@5 proof chip (site-launch.6); the file-organizer story moves below as a use-case; nav += Integrations; FAQ += "works with Claude Code / Codex / MCP?"; copy synced into `web/messages/en.json` — shipped #53: hero.tsx reframe + add-to-agent CTA + "98% hit@5" proof chip (en.json) + nav += Integrations (layout.tsx) (2026-06-14)
- [x] site-growth.4 /integrations hub — grid of every harness (status badges) linking to its page; server-rendered, nav-linked, sitemap entry — shipped #53: `integrations/page.tsx` hub, nav-linked, sitemap entry (2026-06-14)
- [x] site-growth.5 MDX SEO engine — `@next/mdx` + a `gray-matter` glob loader (`web/lib/content.ts`) turning `web/content/{integrations,compare,use-cases,learn}/*.mdx` frontmatter into a typed list; dynamic `[cluster]/[slug]` route with `generateStaticParams` + per-page `generateMetadata` (canonical + OG); `sitemap.ts` auto-extends from the loader (no hand-listing); JSON-LD (SoftwareApplication / FAQPage / HowTo) on templates. en-only for now (`LOCALES=["en"]`) — shipped #53: content.ts loader + `[cluster]/[slug]/page.tsx` + sitemap.ts auto-extend + seo/json-ld.tsx (2026-06-14)
- [~] site-growth.6 Integrations cluster (9 pages) — per-harness MDX sourced from docs/integration.md + .claude-plugin/: claude-code, codex, cursor, pi, opencode, gemini, generic MCP, AI SDK, `dig serve`. #53: per-harness pages generated from `harnesses.ts` via `integrations/[slug]/page.tsx` (8 harnesses) rather than per-harness MDX — implementation diverged from the spec; remaining: a `dig serve` page (9th) + decide MDX-source vs registry-generated (2026-06-14)
- [~] site-growth.7 Comparisons cluster (7 pages) — dig vs mem0 · Letta/MemGPT · LlamaIndex · Obsidian · Zep · vector-DB · RAG-SaaS, sourced from docs/landscape.md (already written). #53: 3/7 shipped — mem0, letta-memgpt, obsidian; remaining: LlamaIndex, Zep, vector-DB, RAG-SaaS (2026-06-14)
- [~] site-growth.8 Concepts/learn cluster (10 pages) — agent memory, MCP, hybrid retrieval (FTS+vector RRF), content-addressed store, drift detection, recall@k/hit@5, local-first AI, reversible changesets, …; sourced from architecture.md + evals.md. #53: 6 MDX shipped — agent-memory, hybrid-retrieval, what-is-mcp + SDK guides (python-sdk, typescript-sdk, vercel-ai-sdk); remaining concepts: content-addressed store, drift detection, recall@k/hit@5, local-first AI, reversible changesets (2026-06-14)
- [~] site-growth.9 Use-cases cluster (8 pages) — agent memory, RAG over local files, dedupe a doc library, organize an Obsidian vault, PARA/GTD/Zettelkasten, research notes, meeting transcripts, second brain. #53: 2/8 shipped — agent-memory, rag-local-files; remaining: dedupe library, Obsidian vault, PARA/GTD/Zettelkasten, research notes, meeting transcripts, second brain (2026-06-14)
