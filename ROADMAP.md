...
- [~] integrations.2 TypeScript/JS SDK — `@dig/client` (clients/typescript): dependency-free typed client over the `dig serve` daemon (find/recall/retain/drift/log/export/org/reconcile/undo, apply-gated; recall/retain make it a memory client) — typed `RecallPack`; built + tested in CI against a real spawned daemon (no mocks) (recall/retain added 2026-06-13). Remaining: AI SDK `tool()` helpers
...
**Exit criteria:** `npm i @dig/client`, `pip install dig-client`, and `brew install dig` all resolve; `dig mcp` is in the public MCP registry; dig appears in the LangChain / LlamaIndex / Vercel AI SDK directories; every entry links the benchmark leaderboard.

- [~] distribution.1 SDK registries — npm + PyPI publish CI wired (npm-publish.yml → `@dig/client`, pypi-publish.yml → `dig-client`; both build dig, test the SDK against a real daemon, publish on a GitHub release; gated on NPM_TOKEN / PYPI_TOKEN secrets). Remaining: add the tokens + first publish of each (2026-06-12)
- [~] distribution.6 Canary channel — `canary.yml` ships bleeding-edge builds on every push to `main`: a rolling `canary` GitHub prerelease of the cross-compiled CLI (GoReleaser snapshot, sha-stamped `dig --version`), `@dig/client@canary` (npm), and `dig-client` `.devN` (PyPI). The CLI canary runs on `GITHUB_TOKEN` alone; the npm/PyPI canaries publish via OIDC trusted publishing, gated on the `CANARY_ENABLED` repo variable until trusted publishers are configured. Stable stays tag-driven. Validated: GoReleaser snapshot + actionlint. Runbook in [docs/RELEASING.md](docs/RELEASING.md). Remaining: enable OIDC + flip `CANARY_ENABLED` (2026-06-13)
...
- [ ] distribution.5 Homebrew tap — `brew install dig` via a `vllnt/homebrew-dig` tap, formula fed by GoReleaser artifacts (public-release.3); the Mac/Linux CLI default alongside the curl installer (site-launch.3). The three front doors: `brew install dig` · `npm i @dig/client` · `pip install dig-client`
